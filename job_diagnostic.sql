...
--    DBMS_OUTPUT.put_line('DEBUG:  write_xml: BEGIN');
  v_xml_str := v_xml_str || '<!-- ######BEGIN DB DX SUMMARY######' || chr(13) || chr(10);
...
--        DBMS_OUTPUT.put_line('DEBUG:  write_xml: QUERY LOOP j=['||j||']');
  ...
  v_xml_str := '';
  v_xml_str := v_xml_str || '</DATA_SET>' || chr(13) || chr(10);
  v_xml_str := v_xml_str || '<SRDC_DIAGNOSTIC_NAME>'||p_script_selected||'</SRDC_DIAGNOSTIC_NAME>' || chr(13) || chr(10);
  v_xml_str := v_xml_str || '</diagnostic>' || chr(13) || chr(10);
  v_xml_str := v_xml_str || ' ######END DB DX SUMMARY######-->' || chr(13) || chr(10);
  dbms_output.put_line(v_xml_str);
...
write_xml(msg_num,script_selected);     
write_html_end;  
