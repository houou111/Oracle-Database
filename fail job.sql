...
select 'db_DHSXKD_failjob_X7Q_' ||to_char(sysdate,'DDMONYYYY') || '.html' file_name from dual;
...
select a.owner,a.job_name, count(1) fail_number,a.enabled
from dba_scheduler_jobs a, DBA_SCHEDULER_JOB_LOG b
where a.job_name=b.job_name
AND status='FAILED'
AND enabled='TRUE'
AND a.OWNER <>'SYS'
GROUP BY a.owner,a.job_name,a.enabled
HAVING count(1) >10
order by 3 desc;
