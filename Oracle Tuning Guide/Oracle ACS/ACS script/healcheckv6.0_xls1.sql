...
spool DBHeathcheck_DBStorage_TopRpt_&&dbname&&timestamp&&suffix 
...
with 
     ...
     v_segstats as
       (select vss.*, 
         ...
         pivot (sum (value) for statistic_name in ('physical reads' as "PR", 
                                                   'physical reads direct' as "PRD", 
                                                   'physical writes' as "PW" , 
                                                   'physical writes direct' as "PWD")) vss
       ),
     v_dbasegs as
       (select 
...
select rpad('=',55,'=') from dual union all
select statline||value
  from (select 1 rn, 
               '&db_name'     dbname,
               to_char(&inst_cnt, '99')    instcnt,
               '&report_time' reptime,
               (select rtrim(xmlagg(xmlelement(e, username||',')).extract('//text()'), ',') 
                  from dba_users 
                 where username in (&schema_exclusion_list)) sel,
               ' ' as sep,  -- separator line
               to_char( &total_db_pr, '999,999,999,999') tpr, 
               to_char( &total_db_pw, '999,999,999,999') tpw,
               to_char( &total_db_pr + &total_db_pw, '999,999,999,999') tio,
               to_char(round( &total_db_pr * 100 / ( &total_db_pr + &total_db_pw ), 2), '999.00') total_pr_2_pw_ratio,
               ' ' as sep1,  -- separator line
               to_char( &segs_with_IO_rep_size_gb, '9,999,999.00') segswIOrszgb,
               to_char( &segs_with_IO_size_gb, '9,999,999.00')     segswIOszgb,
               to_char( &total_db_size_gb, '9,999,999.00')         tdbszgb,
               to_char( &disk_space_alloc + &temp_space_alloc, '9,999,999.00')||'    (including'||
               to_char( &temp_space_alloc, '99999.00')||' GB of Temp space)'   totdskallocgb,
               ' ' as sep2,  -- separator line
               '&startup_time' sttm,
               to_char(&uptime_sec,'999,999,999') upts
          from dual )
 unpivot ( value for statline in (
                              reptime as 'Report generated on:           ', 
                               dbname as 'Database name:                 ',
                              instcnt as 'Number of instances:         ',
                                  sel as 'Schemas excluded from report:  ',
                                  sep as '', -- separator line
                                  tpr as 'Total # of segment Reads:    ', 
                                  tpw as 'Total # of segment Writes:   ',
                                  tio as 'Total # of segment IOs (R+W):',
	          total_pr_2_pw_ratio as 'Reads vs Total segment IOs (R+W), %:  ',
                                 sep1 as '', -- separator line                                   
                         segswIOrszgb as 'Segments generating &v_top_pio_perc% of all IOs, GB:',
                          segswIOszgb as 'All segments with IOs. Total size, GB: ',
                              tdbszgb as 'Total DB size, GB:                     ',
                        totdskallocgb as 'Total disk space allocated, GB:        ',
                                 sep2 as '', -- separator line
                                 sttm as 'Oldest instance startup time:     ',
                                 upts as 'Oldest instance uptime (seconds):         ')
 )
union all
