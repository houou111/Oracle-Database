...
SET TERMOUT OFF
SET DEFINE ON
REM     ORACLE - License Management Services
...
REM               - for Oracle 9i or higher: SELECT ANY DICTIONARY
REM               - when DATABASE VAULT is in use: PARTICIPANT or OWNER authorization on "Oracle Database Vault Realm" is needed (DVSYS user, for example, is a valid choice)
REM          Hint: SYS or SYSTEM are good choices
REM          Example:
REM               sqlplus system/password@orcl1prod
REM
REM        - Run the script:
REM               @ReviewLite15.1.sql
REM
REM        - Where to run the script?
REM               - for RAC databases, connect and run the script on each instance
REM               - for Container Databases (CDB), connect and run the script on CDB$ROOT container and on each open Pluggable Database (PDB)
...
PROMPT GV$PARAMETER
PROMPT ==========================================================
select INST_ID, NAME, VALUE, ISDEFAULT, DESCRIPTION
  from GV$PARAMETER
