...
x := round(dbms_random.value(1,1000));
y := dbms_random.value(1,100000);
cmd := 'delete from THUYNTM_DBA.XML_TEST where NVL("T24LIVE"."NUMCAST"(EXTRACTVALUE(SYS_MAKEXML("SYS_NC00003$"),''/row/c6'')),0) = '||x;
execute immediate (cmd);
--DBMS_OUTPUT.put_line (cmd);
COMMIT; 
END LOOP; 
...
cmd := 'select count(*) from THUYNTM_DBA.XML_TEST where NVL("T24LIVE"."NUMCAST"(EXTRACTVALUE(SYS_MAKEXML("SYS_NC00003$"),''/row/c6'')),0) = '||x;
...
LOOP 
cmd :='UPDATE THUYNTM_DBA.XML_TEST SET XMLRECORD=XMLTYPE(''<row id="'||
