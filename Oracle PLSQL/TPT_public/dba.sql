...
accept dba_response char prompt "Enter Y to find the segment using V$BH (this may take CPU time): "
...
and     lower(trim('&dba_response')) = 'y'
order by
	tch asc
/

accept dba_response char prompt "Enter Y to query what segment resides there (this is IO intensive): "

select owner, segment_name, partition_name, tablespace_name 
from dba_extents
where file_id = &v_dba_rfile
