...
BEGIN
   ...
   LOOP
      FOR c_partition
         IN (SELECT *
             FROM ( SELECT partition_name, default_tablespace
					...
					AND a.blocks  >0
                                        AND  (((a.num_rows/a.blocks) < (d.abc/2) AND a.num_rows>1000)
					    OR a.num_rows=0 )
                                        )
                        ) 
      LOOP
	...
	cmd :='BEGIN DBMS_STATS.gather_table_stats(OwnName =>'''||v_tab.OWNER||''',TabName =>'''||v_tab.table_name||''',PartName =>'''|| c_partition.PARTITION_NAME||''',GRANULARITY => ''AUTO'',degree =>4,estimate_percent=> DBMS_STATS.AUTO_SAMPLE_SIZE,CASCADE => TRUE ); END;';
         ...
         LOOP
            cmd1 := 'alter index '|| c_index.INDEX_OWNER|| '.'|| c_index.index_name
               || ' rebuild partition '|| c_index.PARTITION_NAME|| ' TABLESPACE '|| c_partition.DEFAULT_TABLESPACE|| ' PARALLEL 4';
         ...
         LOOP
            cmd1 := 'alter index '|| c_index.OWNER|| '.'|| c_index.index_name
               || ' rebuild TABLESPACE '|| c_partition.DEFAULT_TABLESPACE|| ' PARALLEL 4';
                 DBMS_OUTPUT.put_line ('-------------------------------------------------------------------');
            DBMS_OUTPUT.put_line (cmd1);
                        execute immediate cmd1;
