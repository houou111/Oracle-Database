...
BEGIN
	...
	LOOP
		cmd :='ALTER TABLE "'|| v_tab.OWNER||'".'|| v_tab.table_name|| ' MOVE TABLESPACE '|| v_tab.DEFAULT_TABLESPACE|| ' COMPRESS FOR QUERY HIGH PARALLEL 4';
		...
		LOOP
            cmd1 := 'alter index '|| c_index.OWNER|| '.'|| c_index.index_name || ' rebuild TABLESPACE '|| v_tab.DEFAULT_TABLESPACE|| ' PARALLEL 4';
            DBMS_OUTPUT.put_line ('-------------------------------------------------------------------');
